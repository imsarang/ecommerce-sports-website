...
exports.mainSearchPage = catchAsyncErrors(async (req, res) => {
    ...
    if (num == null) num = 1
    let { category1, category2 } = req.params
...
exports.allProductCategory = catchAsyncErrors(async (req, res) => {
    const page = req.query.page
    ...
    if (sortMethod == 'highlow') {
        product = await Product.find(keyObj)
            .limit(limit)
            .skip(page * limit)
            .sort({"price":-1})
        total = await Product.find(keyObj).countDocuments()
    }
    else if(sortMethod=='lowhigh'){
        product = await Product.find(keyObj)
            .limit(limit)
            .skip(page * limit)
            .sort({"price":1})
